...
def start_monitor(server, stop_price, limit_price, email, password):
    timer = threading.Timer(10, start_monitor, [server, stop_price, limit_price, email, password])
    timer.start()
    
    price = random.uniform(0, 9)
    message = "Reached price " + str(price)
    
    print (message)
    if not stop_event.is_set():
        if (price <= stop_price):
            message = "Reached stop price " + str(price)
        ...
        if (price <= limit_price):
            message = "Reached limit price " + str(price)
            server.sendmail('Markets Data Intelligence', '4168733699@msg.telus.com', message)
            limit_event.set()
...
start_monitor(server, stop_price, limit_price, settings["email"], settings["email_password"])
