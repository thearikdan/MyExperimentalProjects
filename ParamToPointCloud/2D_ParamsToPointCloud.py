...
DATA_DIR = '/media/ara/HDD/MyProjects/primitives_to_point_clouds/data/square/1500_128'
BATCH_SIZE = 32
...
PROJECT_NAME = "PPC2_2D_SQUARE_1500_128"

OUTPUT_DIR = "output/"+ PROJECT_NAME + "/square"
SUM_DIR = "summary/" + PROJECT_NAME + "/square"
...
#    layer1 = tf.layers.dense(inputs=tf_pc_params, units=5, activation=tf.nn.relu, name = "fc1")
...
out_a = "out_act_"
for i in range(DIMS):
    f_name = out_name + str(i)
    out_k_name = out_k + str(i)
    out_b_name = out_b + str(i)
    out_a_name = out_a + str(i)

    with tf.name_scope(f_name):
#        dim_layer = tf.layers.dense(inputs=dropout1, units=POINT_COUNT, activation=tf.nn.relu, name = f_name)
#        dim_layer = tf.layers.dense(inputs=dropout2, units=POINT_COUNT, activation=tf.nn.relu, name=f_name)
        dim_layer = tf.layers.dense(inputs=layer1, units=POINT_COUNT, activation=tf.nn.relu, name = f_name)
        out_vars = tf.get_collection(tf.GraphKeys.TRAINABLE_VARIABLES, f_name)
        tf.summary.histogram(out_k_name, out_vars[0])
        tf.summary.histogram(out_b_name, out_vars[1])
        tf.summary.histogram(out_a_name, dim_layer)


        outs.append(dim_layer)
...
'''
            data_file = output_dir + "/Debug/data_" + str(i) + ".csv"
