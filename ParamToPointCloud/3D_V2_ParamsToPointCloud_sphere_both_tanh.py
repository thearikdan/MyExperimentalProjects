...
PROJECT_NAME = "PPC2_V2_3D_SPHERE_1500_1024_BOTH_TANH"
...
with tf.device('/gpu:1'):
	...
	#    layer1 = tf.layers.dense(inputs=tf_pc_params, units=PARAMS_VECTOR_SIZE, activation=tf.nn.relu, name = "fc1")
            ...
            mean0, variance0 = tf.nn.moments(tf_pc_params, axes=[0])
            bn0 = tf.nn.batch_normalization(tf_pc_params, mean0, variance0, None, None, variance_epsilon)


	    layer1_t = tflearn.layers.core.fully_connected(bn0, PARAMS_VECTOR_SIZE, activation=tf.nn.tanh, weight_decay=1e-4,
		                                           regularizer='L2', name = "fc1")
            #Restore after batch normalization?    
            layer1 = tf.multiply(layer1_t, tf.sqrt(variance0)) + mean0
	...
	#    dim_layer = tf.layers.dense(inputs=dropout1, units=POINT_COUNT * DIMS, activation=tf.nn.relu, name = out_name)
            ...
            mean1, variance1 = tf.nn.moments(dropout1, axes=[0])
            bn = tf.nn.batch_normalization(dropout1, mean1, variance1, None, None, variance_epsilon)


	    dim_layer_t = tflearn.layers.core.fully_connected(bn, POINT_COUNT * DIMS, activation=tf.nn.tanh, weight_decay=1e-4,
		                                     regularizer='L2', name=out_name)

            #Restore after batch normalization?    
            dim_layer = tf.multiply(dim_layer_t, tf.sqrt(variance1)) + mean1
