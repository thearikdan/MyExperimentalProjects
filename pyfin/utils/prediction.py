...
def get_closest_distances_time_to_predict_and_distances(data_dir, symbol, start_date, end_date, days_count):
    date_time, date_time_per_list, _, _, (close, close_per, close_per_list, dist_close_per_list), _, _ = percentage.get_percentage_change_distance_data(data_dir, symbol, start_date, end_date, days_count)
...
def get_linear_interpolation_prediction(data_dir, symbol, from_date_time, to_date_time, days_count_to_analyse, top_days_count_to_interpolate):
    
    is_data_available, date_time_curr, volume_curr , opn_curr, close_curr, high_curr, low_curr = read.get_intraday_data(data_dir, symbol, from_date_time, to_date_time, 1)
    ...
    start, end, dist = get_closest_distances_time_to_predict_and_distances(data_dir, symbol, from_date_time, to_date_time, days_count_to_analyse)
    ...
    for i in range (count):
        is_data_available, date_time, volume , opn, close, high, low = read.get_intraday_data(data_dir, symbol, start[i], end[i], 1)
